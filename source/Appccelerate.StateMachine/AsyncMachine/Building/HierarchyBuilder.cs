...
namespace Appccelerate.StateMachine.AsyncMachine.Building
{
        ...
        where TEvent : notnull
    {
        ...
        private readonly IDictionary<TState, BuildableStateDefinition<TState, TEvent>> initiallyLastActiveStates;

        public HierarchyBuilder(
            TState superStateId,
            ImplicitAddIfNotAvailableStateDefinitionDictionary<TState, TEvent> stateDefinitions,
            IDictionary<TState, BuildableStateDefinition<TState, TEvent>> initiallyLastActiveStates)
        {
        ...
        private void CheckThatStateHasNotAlreadyASuperState(BuildableStateDefinition<TState, TEvent> subState)
        {
            ...
            if (subState.SuperState != null)
            {
                throw new InvalidOperationException(
                    BuildingExceptionMessages.CannotSetStateAsASuperStateBecauseASuperStateIsAlreadySet(
                        this.superState.Id,
                        subState));
            }
        }
